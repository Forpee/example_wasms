...
fn validate_inputs(total_produced: u32, total_consumed: u32, device_count: u32) -> bool {
    if device_count == 0 {
        return false;
    }
...
fn simulate_historical_usage(total_consumed: u32) -> u32 {
    let past_values = [
...
fn compute_line_losses(total_produced: u32, historical_usage: u32) -> u32 {
...
fn compute_overhead_adjustment(total_consumed: u32) -> u32 {
...
fn per_device_metric(value: u32, device_count: u32) -> u32 {
...
fn simulate_battery(net_energy: u32, historical_usage: u32) -> u32 {
...
fn apply_peak_usage_penalty(net_energy: u32, overhead_adjusted_consumption: u32) -> u32 {
...
fn apply_regulatory_adjustments(cost_per_device: u32) -> u32 {
    let adjustment_a = 20;
    let adjustment_b = 5;
...
fn apply_quality_factor(net_energy: u32, historical_usage: u32, device_count: u32) -> u32 {
    ...
    let with_fixed_sub = safe_sub(safe_add(half_base, 10), 5);
    let quality_deduction = safe_sub(safe_add(with_fixed_sub, 5), 5);

    safe_sub(net_energy, quality_deduction)
}

fn apply_off_peak_rebate(net_energy_after_quality: u32) -> u32 {
    if net_energy_after_quality > 2000 {
        ...
        safe_sub(safe_add(step2, 5), 5)
    }
}

fn apply_auditing_adjustments(final_cost: u32) -> u32 {
    let layer1 = 10;
    let layer2 = 15;
    let layer3 = 5;
